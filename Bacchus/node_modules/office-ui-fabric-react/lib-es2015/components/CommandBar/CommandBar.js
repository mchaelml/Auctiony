...
var CommandBar = /** @class */ (function (_super) {
    ...
    function CommandBar(props) {
        ...
        _this._searchSurface = createRef();
        _this._commandSurface = createRef();
        _this._commandBarRegion = createRef();
        _this._farCommandSurface = createRef();
        _this._focusZone = createRef();
        _this._overflow = createRef();
        _this._onOverflowClick = function (ev) {
            if (_this.state.expandedMenuItemKey === OVERFLOW_KEY) {
                ...
                _this.setState({
                    ...
                    contextualMenuProps: { items: _this.state.renderedOverflowItems },
        ...
        _this._onContextMenuDismiss = function (ev) {
            ...
            if (!ev || !ev.relatedTarget || commandSurface && !commandSurface.contains(ev.relatedTarget)) {
                ...
                if (contextualMenuProps && contextualMenuProps.onDismiss) {
                    _this.state.contextualMenuProps.onDismiss(ev);
                }
                _this.setState({
    ...
    CommandBar.prototype.render = function () {
        ...
        if (isSearchBoxVisible) {
            searchBox = (React.createElement("div", { className: css('ms-CommandBarSearch', styles.search), ref: this._searchSurface },
                React.createElement("input", { className: css('ms-CommandBarSearch-input', styles.searchInput), type: 'text', placeholder: searchPlaceholderText }),
                React.createElement("div", { className: css('ms-CommandBarSearch-iconWrapper ms-CommandBarSearch-iconSearchWrapper', styles.searchIconWrapper, styles.searchIconSearchWrapper) }, React.createElement(Icon, { iconName: 'Search' })),
                React.createElement("div", { className: css('ms-CommandBarSearch-iconWrapper ms-CommandBarSearch-iconClearWrapper', FontClassNames.small, styles.searchIconWrapper, styles.searchIconClearWrapper) },
        ...
        var posInSet = 1;
        return (React.createElement("div", { className: css('ms-CommandBar', styles.root, className), ref: this._commandBarRegion },
            searchBox,
            React.createElement(FocusZone, { componentRef: this._focusZone, className: styles.container, direction: FocusZoneDirection.horizontal, role: 'menubar' },
                React.createElement("div", { className: css('ms-CommandBar-primaryCommands', styles.primaryCommands), ref: this._commandSurface }, renderedItems.map(function (item) { return (_this._renderItemInCommandBar(item, posInSet++, setSize, expandedMenuItemKey)); }).concat((renderedOverflowItems && renderedOverflowItems.length) ? [
                    React.createElement("div", { className: css('ms-CommandBarItem', styles.item), key: OVERFLOW_KEY, ref: this._overflow },
                        React.createElement("button", { type: 'button', id: this._id + OVERFLOW_KEY, className: css('ms-CommandBarItem-link', styles.itemLink, (_c = {},
                            ...
                            React.createElement(Icon, { className: css('ms-CommandBarItem-overflow', styles.itemOverflow), iconName: 'more' })))
                ] : [])),
                React.createElement("div", { className: css('ms-CommandBar-sideCommands', styles.sideCommands), ref: this._farCommandSurface }, renderedFarItems.map(function (item) { return (_this._renderItemInCommandBar(item, posInSet++, setSize, expandedMenuItemKey, true)); }))),
            (contextualMenuProps) ?
                (React.createElement(ContextualMenu, tslib_1.__assign({ className: css('ms-CommandBar-menuHost'), directionalHint: 7 /* bottomAutoEdge */ }, contextualMenuProps, { target: contextualMenuTarget, labelElementId: expandedMenuId, onDismiss: this._onContextMenuDismiss }))) : (null)));
        var _c;
    };
    ...
    CommandBar.prototype._renderItemInCommandBar = function (item, posInSet, setSize, expandedMenuItemKey, isFarItem) {
        var _this = this;
        if (item.onRender) {
            return (React.createElement("div", { className: css('ms-CommandBarItem', styles.item, item.className), key: item.key, ref: item.key }, item.onRender(item, this._onContextMenuDismiss)));
        }
        var itemKey = item.key || String(posInSet);
        var isLink = item.onClick || hasSubmenu(item);
        var className = css(isLink ? ('ms-CommandBarItem-link ' + styles.itemLink) : ('ms-CommandBarItem-text ' + styles.itemText), !item.name && ('ms-CommandBarItem--noName ' + styles.itemLinkIsNoName), (expandedMenuItemKey === item.key) && ('is-expanded ' + styles.itemLinkIsExpanded), item.inactive ? styles.inactive : '');
        var tooltipContent = '';
        if (item.title) {
        ...
        if (item.href) {
            ...
            command = (React.createElement("div", tslib_1.__assign({}, nativeProps, { id: this._id + item.key, className: className, title: '', "aria-disabled": item.inactive, "data-command-key": itemKey, "aria-haspopup": itemHasSubmenu, "aria-label": ariaLabel, "aria-setsize": setSize, "aria-posinset": posInSet }),
                ...
                (isNameVisible) && (React.createElement("span", { className: css('ms-CommandBarItem-commandText', styles.itemCommandText), "aria-hidden": 'true', role: 'presentation' }, item.name))));
        }
        if (item.iconOnly && item.name) {
            command = (React.createElement(TooltipHost, { content: item.name }, command));
        }
        else if (tooltipContent) {
            command = (React.createElement(TooltipHost, { content: tooltipContent }, command));
        }
        return (React.createElement("div", { className: css('ms-CommandBarItem', styles.item, item.className), key: itemKey, ref: itemKey }, command));
    };
    CommandBar.prototype._renderIcon = function (item) {
        ...
        var iconClassName = css('ms-CommandBarItem-icon', styles.itemIcon, iconColorClassName, iconProps.className);
        return React.createElement(Icon, tslib_1.__assign({}, iconProps, { className: iconClassName }));
    };
    ...
    CommandBar.prototype._onItemClick = function (item) {
        ...
        return function (ev) {
            ...
            if (item.key === _this.state.expandedMenuItemKey || !hasSubmenu(item)) {
                _this._onContextMenuDismiss();
            }
            else {
                _this.setState({
